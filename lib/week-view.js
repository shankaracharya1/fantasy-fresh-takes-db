...
export function getWeekWindowFromReference(referenceDate = todayInIstYmd()) {
  ...
  const sundayOffset = -weekday;   // always go back to Sunday
  const weekStart = shiftYmd(safeReference, sundayOffset);
...
export function getMonthWeekDateRange(monthKey, weekInMonth) {
  ...
  // Anchor: first day of week N in the month (e.g. week 3 → day 15)
  const anchorDay = (safeWeek - 1) * 7 + 1;
  const anchorDate = new Date(Date.UTC(year, month - 1, anchorDay, 12));

  // Snap back to Sunday (getUTCDay: 0=Sun, 6=Sat)
  const weekday = anchorDate.getUTCDay();
  const sunday = new Date(Date.UTC(year, month - 1, anchorDay - weekday, 12));
  const saturday = new Date(Date.UTC(year, month - 1, anchorDay - weekday + 6, 12));

  return {
    start: formatYmd(sunday),
    end: formatYmd(saturday),
  };
}
...
// Generates all calendar weeks from sinceDate up to today (IST), independent of data.
export function buildStaticMonthWeekOptions(sinceDate = MIN_DASHBOARD_DATE) {
  const today = todayInIstYmd();
  const currentMonthKey = today.slice(0, 7);
  const currentDay = Number(today.slice(8, 10));
  const currentWeek = Math.min(4, Math.floor((currentDay - 1) / 7) + 1);

  const sinceMonthKey = isValidYmd(sinceDate) ? sinceDate.slice(0, 7) : MIN_DASHBOARD_DATE.slice(0, 7);
  const sinceDay = isValidYmd(sinceDate) ? Number(sinceDate.slice(8, 10)) : 1;
  const sinceWeek = Math.min(4, Math.floor((sinceDay - 1) / 7) + 1);

  const options = [];
  let [yr, mo] = sinceMonthKey.split("-").map(Number);

  while (true) {
    const mk = `${yr}-${String(mo).padStart(2, "0")}`;
    if (mk > currentMonthKey) break;

    const isFirst = mk === sinceMonthKey;
    const isLast = mk === currentMonthKey;
    const startW = isFirst ? sinceWeek : 1;
    const endW = isLast ? currentWeek : 4;

    for (let w = startW; w <= endW; w++) {
      const range = getMonthWeekDateRange(mk, w);
      if (!range) continue;
      options.push({
        id: `${mk}::${w}`,
        monthKey: mk,
        weekInMonth: w,
        label: formatMonthWeekLabel(mk, w),
        weekStart: range.start,
        weekEnd: range.end,
        rangeLabel: formatWeekRangeLabel(range.start, range.end),
      });
