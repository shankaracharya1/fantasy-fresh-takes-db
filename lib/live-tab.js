...
import { formatWeekRangeLabel, getWeekSelection, normalizeWeekView, shiftYmd, todayInIstYmd } from "./week-view.js";
...
export const LIVE_TAB_NAME = "Live_old";
export const EDITORIAL_TAB_NAME = "Editorial";
export const PRODUCTION_TAB_NAME = "Production";
export const IDEATION_TAB_NAME = "Ideation running list";
...
const ANALYTICS_LIVE_TAB_COL_INDEX = {
  ...
  video75To95Pct: 84,
  video0To95Pct: 85,
  thruPlayTo3sRatio: 86,
  absoluteCompletionPct: 87,
  cpmUsd: 89,
  cpiUsd: 90,
  ctrPct: 91,
  amountSpentUsd: 92,
  outboundClicksToCompletionPct: 93,
  reach: 94,
  impressions: 95,
  clickToInstall: 96,
};
...
function normalizeIdeationWeekLabel(value) {
  return normalizeText(value)
...
function getIdeationWeekBucket(weekSelection) {
...
function normalizeLiveTabRow(rawRow, rowIndex) {
  ...
  return {
    ...
    liveDate: parseLiveDate(row[LIVE_TAB_COL_INDEX.liveDate]),
  };
}
...
function normalizeAnalyticsLiveTabRow(rawRow, rowIndex) {
  ...
  const normalizedRow = {
    ...
    liveDate: parseLiveDate(row[ANALYTICS_LIVE_TAB_COL_INDEX.liveDate])
...
function normalizeIdeationTabRow(rawRow, rowIndex) {
  ...
  return {
    rowIndex,
    ideationWeek: normalizeText(row[IDEATION_TAB_COL_INDEX.ideationWeek]),
...
export function buildGoodToGoBeatsMetricsFromIdeationTab(rows, period = "current", options = {}) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    const ideationWeek = normalizeIdeationWeekLabel(row?.ideationWeek);
    const status = normalizeKey(row?.status);
    if (!ideationWeek || ideationWeek !== normalizedBucket) {
      continue;
    }

    if (status === "gtg" || status === "gtg - minor changes") {
