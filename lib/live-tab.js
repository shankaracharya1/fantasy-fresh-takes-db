...
function normalizeIdeationTabRow(rawRow, rowIndex, headers) {
  ...
  return {
    ...
    scriptStatus: normalizeText(getHeaderCellPrefix(row, headers, ["Script status", "Script Status"])),
  };
}
...
// Like getHeaderCell but matches headers that START WITH any candidate (handles headers with extra descriptive text)
function getHeaderCellPrefix(row, headers, candidates) {
  ...
  for (const candidate of Array.isArray(candidates) ? candidates : []) {
    ...
    const index = normalizedHeaders.findIndex((h) => h === nc || h.startsWith(nc + " "));
