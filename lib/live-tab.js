...
export async function fetchLiveWorkflowRows() {
  const result = await fetchTrackerTabRowsByHeaders(LIVE_TAB_NAME, (rawRow, rowIndex, headers) => {
    ...
    return {
      ...
      video0To25Pct: getHeaderCell(row, headers, ["Video - 0% - 25%", "Q1 Completion"]),
      ...
      absoluteCompletionPct: getHeaderCell(row, headers, ["Completions/ Impressions", "Completions / Impressions", "True Completion"]),
      ...
      amountSpentUsd: getHeaderCell(row, headers, ["Amount spent (USD)", "Amount Spent", "Spend"]),
      ...
      clickToInstall: getHeaderCell(row, headers, ["Click to Install", "CTI"]),
