...
export function buildGoodToGoBeatsMetricsFromIdeationTab(rows, period = "current", options = {}) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    const beatsStatus = normalizeKey(row?.beatsStatus);
    ...
    if (beatsStatus.includes("review") && beatsStatus.includes("pend")) {
      reviewPendingCount += 1;
    } else if (beatsStatus === "iterate" || beatsStatus.includes("iteration")) {
      iterateCount += 1;
    }
  }
