...
export const POD_LEAD_ORDER = ["Paul", "Josh", "Nishant", "Dan", "Jacob Berman", "Aakash Ahuja", "Yadhu"];
...
const LIVE_TAB_COL_INDEX = {
  ...
  cdName: 28,         // AC "CD" (was AB=27 before column inserted at AA)
...
const ANALYTICS_LIVE_TAB_COL_INDEX = {
  ...
  assetLink: 74,       // BW "Ad Name"
  uploadDate: 76,      // BY — Final Upload Date
  threeSecPlayPct: 79, // CB "3 Sec Play" (was CC=80)
  thruPlaysPct: 80,    // CC "ThruPlays" (was CD=81)
  video0To25Pct: 81,   // CD "Video - 0% - 25%" (was CE=82)
  video25To50Pct: 82,  // CE "Video - 25% - 50%" (was CF=83)
  video50To75Pct: 83,  // CF "Video - 50% - 75%" (was CG=84)
  video75To95Pct: 84,  // CG "Video - 75% - 95%" (was CH=85)
  video0To95Pct: 85,   // CH "Video - 0% - 95%" (was CI=86)
  thruPlayTo3sRatio: 86, // CI "Thruplay/3s" (was CJ=87)
  absoluteCompletionPct: 87, // CJ "Completions/Impressions" (was CK=88)
  cpmUsd: 89,          // CL "CPM" (was CM=90)
  cpiUsd: 90,          // CM "Cost per App Install (USD)" (was CN=91)
  ctrPct: 91,          // CN "CTR" (was CO=92)
  amountSpentUsd: 92,  // CO "Amount spent (USD)" (was CP=93)
  outboundClicksToCompletionPct: 93, // CP (was CQ=94)
  reach: 94,           // CQ "Reach" (was CR=95)
  impressions: 95,     // CR "Impressions" (was CS=96)
  clickToInstall: 96,  // CS "Click to Install" (was CT=97)
  q1ToImpressions: 100, // CW "Q1/Impressions"
};
...
const IDEATION_TAB_COL_INDEX = {
  ...
  writerName: 3,         // D — Beats owner
  beatCode: 3,           // D — beat/code reference (fallback; prefer header detection)
...
const AD_CODE_HEADER_CANDIDATES = [
  "Ad Code",
  "AD Code",
  "F Ad Code",
  "F AD Code",
...
function normalizeAnalyticsLiveTabRow(rawRow, rowIndex, headers) {
  ...
  const assetCode = Array.isArray(headers) && headers.length > 0
    ? normalizeText(getHeaderCell(row, headers, AD_CODE_HEADER_CANDIDATES))
  ...
  const podLeadRaw = Array.isArray(headers) && headers.length > 0
    ? normalizeText(getHeaderCell(row, headers, ["POD", "POD Lead", "POD Name"]))
    : normalizeText(row[ANALYTICS_LIVE_TAB_COL_INDEX.podLead]);
  const reworkType = Array.isArray(headers) && headers.length > 0
    ? normalizeText(getHeaderCell(row, headers, ["Type of rework", "Rework Type", "Rework type", "Script Type", "Script type"]))
    : "";
  const normalizedRow = {
    rowIndex,
    assetCode,
    ...
    podLeadName: normalizePodLeadName(podLeadRaw),
    reworkType,
    ...
    clickToInstall,
    q1ToImpressions: parseMetricPercent(row[ANALYTICS_LIVE_TAB_COL_INDEX.q1ToImpressions]),
  };
...
function normalizeIdeationTabRow(rawRow, rowIndex, headers) {
  ...
  const beatsAssignedDate = normalizeText(getHeaderCell(row, headers, ["Beats week", "Beats assigned date (Cntrl + ; to add today's date > edit if future beats)", "Beats assigned date"])) || normalizeText(row[IDEATION_TAB_COL_INDEX.beatsAssignedDate]);
  ...
  return {
    ...
    writerName: normalizeText(getHeaderCellPrefix(row, headers, ["Beats owner", "Beat owner", "Writer", "Writer Name"])) || normalizeText(row[IDEATION_TAB_COL_INDEX.writerName]),
    ...
    scriptCode: normalizeText(getHeaderCellPrefix(row, headers, ["Script code", "Script Code"])),
    scriptStatus: normalizeText(getHeaderCellPrefix(row, headers, ["Script status", "Script Status"]) || row[15]), // col P
  };
}

const SHEET_CACHE_TTL_MS = 8 * 60 * 60 * 1000;          // 8 hours (was 4h)
const CSV_FETCH_TIMEOUT_MS = 15000;
const SHEETS_API_TIMEOUT_MS = 25000;
const SUPABASE_SHEET_CACHE_TTL_MS = 8 * 60 * 60 * 1000; // 8 hours (was 3h)
...
// Clears in-memory cache for all (or a specific) sheet so the next request hits Google Sheets.
export function evictInMemorySheetCache(sheetName = null) {
  if (sheetName) {
...
// Expires the Supabase persistent cache for a sheet (writes cachedAt=0 so TTL check fails).
export async function expireSupabaseSheetCache(sheetName) {
  try {
    await writeJsonObject(sheetCachePath(sheetName), { rows: [], cachedAt: 0, sheetName });
  } catch {
    // Non-fatal
  }
}
...
// Like getHeaderCell but matches headers that START WITH or CONTAIN any candidate
// (handles headers with extra descriptive text like "Script status (Automated...)")
function getHeaderCellPrefix(row, headers, candidates) {
  const normalizedHeaders = Array.isArray(headers) ? headers.map((h) => normalizeHeaderKey(h)) : [];
  for (const candidate of Array.isArray(candidates) ? candidates : []) {
    const nc = normalizeHeaderKey(candidate);
    // Try exact, then startsWith, then includes — progressively broader
    let index = normalizedHeaders.findIndex((h) => h === nc);
    if (index === -1) index = normalizedHeaders.findIndex((h) => h.startsWith(nc + " "));
    if (index === -1) index = normalizedHeaders.findIndex((h) => h.includes(nc));
    if (index !== -1) return Array.isArray(row) ? (row[index] ?? "") : "";
  }
  return "";
}
...
export async function fetchReadyForProductionTabRows() {
  return fetchTrackerTabRowsByHeaders(READY_FOR_PRODUCTION_TAB_NAME, (rawRow, rowIndex, headers) => {
    const row = Array.isArray(rawRow) ? rawRow : [];
    const assetCodeIndex = findHeaderIndex(headers, AD_CODE_HEADER_CANDIDATES);
...
export async function fetchEditorialWorkflowRows() {
  ...
  return {
    rowIndex,
    assetCode: normalizeText(getHeaderCell(row, headers, AD_CODE_HEADER_CANDIDATES)),
    ...
    writerName: normalizeText(getHeaderCell(row, headers, ["Beats owner", "Writer", "Writer Name"])),
      ...
      dateSubmittedByLead: parseLiveDate(
        getHeaderCellPrefix(row, headers, ["Date submitted by Lead", "Submitted by Lead"]) ||
        row[EDITORIAL_TAB_COL_INDEX.submittedDate]
      ),
...
export async function fetchProductionWorkflowRows() {
  return fetchTrackerTabRowsByHeaders(PRODUCTION_TAB_NAME, (rawRow, rowIndex, headers) => {
    ...
    return {
      ...
      etaToStartProd: parseLiveDate(
        getHeaderCell(row, headers, ["ETA to start prod", "ETA to start production", "Date approved for prod", "Date assigned", "Production picked date"]) ||
      ...
      scriptStatus: normalizeText(getHeaderCell(row, headers, ["Script status", "Script Status", "Status"])) || normalizeText(row[PRODUCTION_TAB_COL_INDEX.status]),
    };
  });
}

export async function fetchLiveWorkflowRows() {
  const result = await fetchTrackerTabRowsByHeaders(LIVE_TAB_NAME, (rawRow, rowIndex, headers) => {
    const row = Array.isArray(rawRow) ? rawRow : [];
    const finalUploadDateRaw =
      getHeaderCell(row, headers, ["Final Upload Date", "Final upload date", "Upload Date"]) ||
      row[LIVE_TAB_COL_INDEX.uploadDate];
    const rawPodLeadName = normalizeText(getHeaderCell(row, headers, ["POD", "POD Lead", "POD Name"]));
    return {
      rowIndex,
      assetCode: normalizeText(getHeaderCell(row, headers, AD_CODE_HEADER_CANDIDATES)),
      ...
      status: normalizeText(getHeaderCell(row, headers, ["Status"])),
      scriptStatus: normalizeText(getHeaderCell(row, headers, ["Script status", "Script Status", "Status"])),
      reworkGaCode: normalizeText(getHeaderCellPrefix(row, headers, ["Script code to be reworked GA code", "Script code to be reworked", "Reworked GA code"])),
      finalUploadDate: parseLiveDate(finalUploadDateRaw),
      liveDate: parseLiveDate(finalUploadDateRaw),
      cpiUsd: parseMetricNumber(getHeaderCell(row, headers, ["Cost per App Install (USD)", "CPI", "CPI (USD)"])),
      threeSecPlayPct: getHeaderCell(row, headers, ["3 Sec Play", "3s", "3 sec play"]),
      thruPlaysPct: getHeaderCell(row, headers, ["ThruPlays", "Thru Plays"]),
      q1ToThruplays: getHeaderCell(row, headers, ["Q1/ thruplays", "Q1/Thruplays"]),
      video0To25Pct: parseMetricPercent(getHeaderCell(row, headers, ["Video - 0% - 25%", "Q1 Completion"])),
      q1: getHeaderCell(row, headers, ["Q1"]),
      video25To50Pct: getHeaderCell(row, headers, ["Video - 25% - 50%"]),
      video50To75Pct: getHeaderCell(row, headers, ["Video - 50% - 75%"]),
      video75To95Pct: getHeaderCell(row, headers, ["Video - 75% - 95%"]),
      video0To95Pct: getHeaderCell(row, headers, ["Video - 0% - 95%"]),
      thruPlayTo3sRatio: parseMetricPercent(getHeaderCell(row, headers, ["Thruplay/3s", "ThruPlay/3s"])),
      absoluteCompletionPct: parseMetricPercent(getHeaderCell(row, headers, ["Completions/ Impressions", "Completions / Impressions", "True Completion"])),
      q1ToImpressions: parseMetricPercent(getHeaderCell(row, headers, ["Q1/Impressions"])),
      q4ToQ1: getHeaderCell(row, headers, ["Q4/Q1"]),
      cpmUsd: getHeaderCell(row, headers, ["CPM (cost per 1,000 impressions) (USD)", "CPM"]),
      ctrPct: parseMetricPercent(getHeaderCell(row, headers, ["CTR (link click-through rate)", "CTR"])),
      amountSpentUsd: parseMetricNumber(getHeaderCell(row, headers, ["Amount spent (USD)", "Amount Spent", "Spend"])),
      outboundClicksToCompletionPct: getHeaderCell(row, headers, ["Outbound Clicks to Completion"]),
      reach: getHeaderCell(row, headers, ["Reach"]),
      impressions: getHeaderCell(row, headers, ["Impressions"]),
      clickToInstall: parseMetricPercent(getHeaderCell(row, headers, ["Click to Install", "CTI"])),
      ctrTimesCti: getHeaderCell(row, headers, ["CTR*CTI", "CTR x CTI"]),
      appInstalls: getHeaderCell(row, headers, ["App Installs", "App installs"]),
    };
  });
  return result;
}
