...
function normalizeIdeationTabRow(rawRow, rowIndex, headers) {
  ...
  return {
    ...
    assignedDate: normalizeText(getHeaderCell(row, headers, ["Beats assigned date (Cntrl + ; to add today's date > edit if future beats)", "Beats assigned date", "Assigned date", "Date assigned"])) || normalizeText(row[IDEATION_TAB_COL_INDEX.assignedDate]),
    completedDate: normalizeText(getHeaderCell(row, headers, ["Beats completed (Cntrl + ; to add today's date > edit if future beats)", "Beats completed", "Completed date", "Date completed"])) || normalizeText(row[IDEATION_TAB_COL_INDEX.completedDate]),
    beatsAssignedDate: normalizeText(getHeaderCell(row, headers, ["Beats assigned date (Cntrl + ; to add today's date > edit if future beats)", "Beats assigned date"])) || normalizeText(row[IDEATION_TAB_COL_INDEX.beatsAssignedDate]),
...
const CSV_FETCH_TIMEOUT_MS = 20000;
const SHEETS_API_TIMEOUT_MS = 10000;
...
async function fetchTrackerSheetRows(sheetName) {
  ...
  if (cachedFresh) {
    return {
      spreadsheetId,
      parsedRows: cachedFresh.rows,
    };
  }

  const trySheetsApiFirst = hasServiceAccountKeyConfigured();
  const fetchAttempts = trySheetsApiFirst
    ? [() => fetchRowsViaCsv(spreadsheetId, sheetName), () => fetchRowsViaSheetsApi(spreadsheetId, sheetName)]
