...
const SHEET_TAB = "Ideation running list";
const SHEET_RANGE = `${SHEET_TAB}!A:Z`;
const CACHE_TTL_MS = 5 * 60 * 1000;
const ALLOWED_STATUSES = new Set(["GTG", "GTG - minor changes"]);
let beatDocsCache = {
  expiresAt: 0,
  items: null,
};
...
async function fetchBeatDocsFromSheet() {
  ...
  const headerIndexes = findHeaderIndexes(rows[0]?.values || []);

  if (!hasRequiredHeaderIndexes(headerIndexes)) {
