...
const LIVE_COL_INDEX = {
  ...
  imageSheetLinks: 52, // BA "Final Image Sheet" (was AZ=51)
  liveDate: 76 // BY "Final Upload Date" (was BX=75)
};
...
async function fetchLiveTabRows() {
  ...
  console.log(`[acd-live-sheet-names] ${JSON.stringify({ allSheetNames, lookingFor: LIVE_SOURCE_TAB })}`);

  const worksheet = workbook.Sheets[LIVE_SOURCE_TAB];
  if (!worksheet) {
    throw makeError(400, `Live source tab "${LIVE_SOURCE_TAB}" not found. Available tabs: ${allSheetNames.join(", ")}`);
  }
...
async function syncAcdFromLive(options = {}) {
  ...
  const { spreadsheetId, allSheetNames, rows, rowMetaByIndex } = await fetchLiveTabRows();
  ...
  for (let i = 2; i < rows.length; i += 1) {
    ...
    // Collect first 10 data rows for column diagnostics (GA rows only)
    if (isGaAssetCode(assetCode) && columnDiagSampleRows.length < 10) {
      columnDiagSampleRows.push({
        rowIndex: liveRowIndex,
        colB_raw: String(row[LIVE_COL_INDEX.assetCode] === undefined ? "" : row[LIVE_COL_INDEX.assetCode]).slice(0, 60),
        colB_parsed: assetCode,
        colAC_raw: String(row[LIVE_COL_INDEX.cdName] === undefined ? "" : row[LIVE_COL_INDEX.cdName]).slice(0, 60),
        colAC_parsed: cdName,
        colBA_raw: String(row[LIVE_COL_INDEX.imageSheetLinks] === undefined ? "" : row[LIVE_COL_INDEX.imageSheetLinks]).slice(0, 60),
        colBY_raw: String(row[LIVE_COL_INDEX.liveDate] === undefined ? "" : row[LIVE_COL_INDEX.liveDate]).slice(0, 40),
  ...
  if (columnDiagSampleRows.length > 0) {
    console.log(`[acd-live-column-diag] ${JSON.stringify(columnDiagSampleRows)}`);
  }
