...
const CREATIVE_DIRECTOR_ALIASES = new Map(
  [
    ...
    ["Ankit D Bagde", ["ankit d badge", "ankit d bagde", "ankit badge", "ankit"]],
    ...
    ["Hitesh Gawankar", ["hitesh gawankar", "hitesh"]],
    ["Sakshi Nandwani", ["sakshi nandwani", "sakshi"]],
    ["Tanya Singh", ["tanya singh", "tanya"]],
    ["Ankur Saraf", ["ankur saraf", "ankur"]],
...
function toTitleCase(value) {
  ...
  return cleaned.split(" ").map((part) => {
    if (!part) return "";
    // All-uppercase or all-lowercase → force title case (handles THOR, thor → Thor)
    if (/^[A-Z0-9]+$/.test(part) || /^[a-z0-9]+$/.test(part)) {
      return `${part.slice(0, 1).toUpperCase()}${part.slice(1).toLowerCase()}`;
    }
    // Mixed case (e.g. already "Thor") → just ensure first letter is uppercase
...
function normalizeAcdName(value) {
  const cleaned = String(value || "").trim().replace(/\s+/g, " ");
  if (!cleaned) return "";
  return CANONICAL_ACD_ALIASES.get(cleaned.toLowerCase()) || toTitleCase(cleaned);
}
...
function normalizeCdName(value) {
  ...
  return CREATIVE_DIRECTOR_ALIASES.get(aliasKey) || toTitleCase(cleaned);
}
...
function aggregateAcdFromRecords(acdRows) {
  ...
  const start14 = shiftDate(today, -13);
  ...
  for (const row of dailyRows) {
    ...
    if (row.workDate >= start14 && row.workDate <= today) {
      addRollingRow(rolling14Acd, row.acdName, row.totalMinutes, row.totalImages);
      addRollingRow(rolling14Cd, row.cdName, row.totalMinutes, row.totalImages);
    }
  ...
  return {
    ...
    rolling14Rows: mapToRows(rolling14Acd, "acdName"),
    rolling30Rows: mapToRows(rolling30Acd, "acdName"),
    rolling7CdRows: mapToRows(rolling7Cd, "cdName"),
    rolling14CdRows: mapToRows(rolling14Cd, "cdName"),
