...
async function buildBeatsPerformancePayload() {
  ...
  const sourceErrors = {
    live: liveResult?.status === "rejected" ? (liveResult.reason?.message || "Live tab unavailable") : "",
    editorial: editorialResult?.status === "rejected" ? (editorialResult.reason?.message || "Editorial tab unavailable") : "",
    readyForProduction: readyForProductionResult?.status === "rejected" ? (readyForProductionResult.reason?.message || "Ready for Production tab unavailable") : "",
    production: productionResult?.status === "rejected" ? (productionResult.reason?.message || "Production tab unavailable") : "",
    editorialWorkflow: editorialWorkflowResult?.status === "rejected" ? (editorialWorkflowResult.reason?.message || "Editorial workflow unavailable") : "",
    liveWorkflow: liveWorkflowResult?.status === "rejected" ? (liveWorkflowResult.reason?.message || "Live workflow unavailable") : "",
    ideation: beatRowsResult?.status === "rejected" ? (beatRowsResult.reason?.message || "Ideation tracker unavailable") : "",
  };
  const warnings = [
    beatRows.length === 0 ? `Ideation tracker data is unavailable right now.${sourceErrors.ideation ? ` (${sourceErrors.ideation})` : ""}` : "",
    sourceErrors.live ? `Live tab: ${sourceErrors.live}` : "",
    sourceErrors.editorial ? `Editorial tab: ${sourceErrors.editorial}` : "",
    sourceErrors.readyForProduction ? `Ready for Production tab: ${sourceErrors.readyForProduction}` : "",
    sourceErrors.production ? `Production tab: ${sourceErrors.production}` : "",
  ].filter(Boolean);
  ...
  return {
    ...
    rowCounts: {
      ...
      readyForProduction: readyForProductionRows.length,
      production: productionRows.length,
      editorialWorkflow: editorialWorkflowRows.length,
      liveWorkflow: liveWorkflowRows.length,
      ideationRaw: beatRows.length,
      ideationFiltered: filteredBeatRows.length,
    },
    benchmark: {
